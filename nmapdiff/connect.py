"""Native cognis-connect emit for nmapdiff — forward findings to any platform.

Maps nmapdiff's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    nmapdiff ... --format json | nmapdiff-emit --to stix
    nmapdiff-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('host', {}).get('address', {}).get('@addr') or rec.get('host', {}).get('name')
        out['severity'] = 'info'
        out['type'] = 'nmap_diff'
        out['description'] = 'New host/port/service detected in scan difference'
        out['tags'] = ['nmap', 'scan', 'diff']
        out['ipv4'] = rec.get('host', {}).get('address', {}).get('@addr')
        out['domain'] = rec.get('host', {}).get('name')
        out['url'] = ''
        out['sha256'] = ''
        out['cve'] = ''
        out['imo'] = ''
